...
function addNode(root, key, value) {
    if (root.key < key) {
        if (root.right) {
            addNode(root.right, key, value);
        }
        else {
            root.right = { key, values: [value] };
        }
    }
...
function getNodes(root, key) {
    if (root.key === key) {
        return root.values;
    }
    else if (root.key < key) {
        return root.right ? getNodes(root.right, key) : undefined;
    }
    else if (root.key > key) {
        return root.left ? getNodes(root.left, key) : undefined;
    }
    return undefined;
}
/**
